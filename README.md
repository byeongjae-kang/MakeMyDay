...
\*Login is not enabled. To enable login: On Layout.js (front-end folder), change line 96 from `user` to `!user`. To enable Google login, create an OAUTH2 Client ID on [Google Cloud Platform](https://console.cloud.google.com/). In the front-end folder, create a .env.local file by copying "env.local example": `cp env.local example .env.local` and then paste your Google OAUTH2 ID after `REACT_APP_GOOGLE_CLIENT_ID=` in the .env.local file that you made.
