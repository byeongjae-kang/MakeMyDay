...
//       fontSize: "20px"}//The style to be applied to the title
// 		},
// 		task:{// The items inside the list diplaying the task
// 			style:{backgroundColor:  '#fbf9f9', fontSize: "15px"}// the style to be applied
// 		},
...
function Gantt() {
  const {state, updateTask} = useApplicationData();
...
const sortedTasks = state.tasks.sort((a,b) => {
  ...
  return (
    <Container style={{width:'3000px'}}>
      <TimeLine data={state.tasks} config={state.params} onUpdateTask={onUpdateTask} style={{height: '100px'}}/>
    </Container>
  )
}
