...
function ProjectView() {
  ...
  const updateTask = function (id, start_date, end_date, status, priority) {
    axios.put(`http://localhost:8080/api/tasks/${id}`, { status: status, start: start_date, "end": end_date, priority: priority })
      .then(result => {
  ...
  const stateData = { projects, updateTask, updateDragDrop, createTask }
  ...
  return (
    <ProjectContext.Provider value={stateData}>
      ...
      <div>
        ...
        <button onClick={toggleView}>{view ? "Gantt View" : "Project View"}</button>
      </div>
      {view && <TasksBody />}
      {!view && <Gantt />}
